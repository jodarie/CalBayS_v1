...
def Best_fit(Pmatrix,Xmatrix,DHmatrix,Vc,vi,So,bo,dim,nw,nc,lab,nmodel,eArgs):
  ...
  def Nlogprob(p):
    ...
    if ~np.isnan(np.sum(DHm_th)):
      for i in range(nconc):
        DH_th[:,i] = (1.0/CS[i])*((Ptot[1:,i]*DHm_th[1:,i]*(0.5+(Vc/vi[1:,i])))+(Ptot[:-1,i]*DHm_th[:-1,i]*(0.5-(Vc/vi[:-1,i]))))
        if eArgs[0]:
...
def MCMC_analysis(chain,out,Nw,Nc,burn_in):
  ...
  samples= chain[mask]
  outdata = np.array(map(lambda v: (v[1], v[2]-v[1], v[0]-v[1]), zip(*np.percentile(samples, [16,50,84],axis=0))))
  np.savetxt(out+'/MCMC_analysis.dat',outdata,fmt='%s')
  return np.array(map(lambda v: (v[1], v[2]-v[1], v[0]-v[1]), zip(*np.percentile(samples, [16,50,84],axis=0))))
...
def Figure_fit(p,out,Pmatrix,Xmatrix,DHmatrix,Vc,vi,So,color_code,nmodel):
  ...
  for i,Xrow in enumerate(Xmatrix):
    for j,Xtot in enumerate(Xrow):
      DHm_th[i][j] = DHm(Xtot,Kv,Ptot[i][j],DH)
  for i in range(nconc):
    DH_th[:,i] = (1.0/CS[i])*((Ptot[1:,i]*DHm_th[1:,i]*(0.5+(Vc/vi[1:,i])))+(Ptot[:-1,i]*DHm_th[:-1,i]*(0.5-(Vc/vi[:-1,i]))))+q0[i]
